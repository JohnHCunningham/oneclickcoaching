-- ============================================
-- WORKING INTEGRATION SETUP
-- Uses existing account, doesn't create new one
-- ============================================

-- Don't create a new account - use existing one
-- The RPC functions will work with whatever account the user belongs to
...
-- Create RPC functions that find user's account dynamically
...
DECLARE
    ...
    connection_id UUID;
BEGIN
    -- Get the user's account from User_Roles if it exists
    SELECT account_id INTO user_account_id
    FROM "User_Roles"
    WHERE user_id = auth.uid()
    LIMIT 1;

    -- If no User_Roles, get first account owned by user
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Note: Using your existing account(s)';
    RAISE NOTICE '';
END $$;
