...
-- Works without modifying existing tables
-- ============================================
...
-- Create a default account for single-tenant use
INSERT INTO "Accounts" (id, company_name, industry)
VALUES ('00000000-0000-0000-0000-000000000001', 'Default Account', 'Technology')
ON CONFLICT (id) DO NOTHING;

-- Create Conversation_Analyses table
CREATE TABLE IF NOT EXISTS "Conversation_Analyses" (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    account_id UUID REFERENCES "Accounts"(id) ON DELETE CASCADE DEFAULT '00000000-0000-0000-0000-000000000001',
...
CREATE TABLE IF NOT EXISTS "API_Connections" (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    account_id UUID NOT NULL REFERENCES "Accounts"(id) ON DELETE CASCADE DEFAULT '00000000-0000-0000-0000-000000000001',
...
-- Create Synced_Conversations table
...
-- Create Coaching_Outcomes table
...
-- Create simplified RPC functions that use default account
...
BEGIN
    ...
    WHERE ac.account_id = default_account_id AND ac.provider = 'hubspot'
...
BEGIN
    INSERT INTO "API_Connections" (
        ...
        default_account_id, 'fathom', access_token_input, refresh_token_input,
        NOW() + (expires_in_seconds || ' seconds')::INTERVAL,
        auth.uid(), 'active'
    )
    ON CONFLICT (account_id, provider)
...
BEGIN
    ...
    WHERE ac.account_id = default_account_id AND ac.provider = 'fathom'
...
BEGIN
    ...
    RAISE NOTICE '  • Accounts (with default account)';
    RAISE NOTICE '  • Conversation_Analyses';
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Created 4 RPC functions:';
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Note: Using single default account for now.';
    RAISE NOTICE 'Multi-tenant can be added later if needed.';
    RAISE NOTICE '';
END $$;
