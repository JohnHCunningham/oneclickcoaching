-- ============================================
-- INTEGRATION SETUP - NO USER_ROLES DEPENDENCY
-- Works without User_Roles.account_id column
-- ============================================
...
-- Create RPC functions that use ONLY the Accounts table
...
DECLARE
    ...
    connection_id UUID;
BEGIN
    -- Find account owned by current user
    SELECT id INTO user_account_id
    FROM "Accounts"
    WHERE owner_user_id = auth.uid()
    LIMIT 1;

    -- If no account found, use first available account
...
BEGIN
    ...
    RAISE NOTICE '📖 Next: Open integrations.html to connect HubSpot';
    RAISE NOTICE '';
END $$;
